...
There is **no top-level build, no test suite, no linter, no package manager**. Each sub-project is an independent Python tree with its own entry scripts and `SKILL.md` / `CLAUDE.md` / `AGENTS.md` / `README.md`. Configuration varies: the four 制单 projects each have their own `.env`; the four 查询X余额 projects **reuse the matching 制单 project's `.env`** and do not have their own. Some projects ship a `requirements.txt`; others (e.g. 农业银行) document dependencies inline in their `CLAUDE.md`. Always read the specific project's docs before touching it; this file only covers what is true across the whole suite.
...
2. The target project's own `SKILL.md` / `CLAUDE.md` / `AGENTS.md` / `README.md` — these are the source of truth for that project's specifics.
...
| 中国银行 (`中行/中国银行`) | `启动.bat` / `open_boc.py` / `batch_run_screenshots.py` | `BOC_ENABLE_TRANSFER_FILL=1` to prefill required fields; `BOC_ENABLE_ORDER_SUBMIT=1` to click the bottom `提交` **once and stop**. `BOC_AUTO_USHIELD_PIN=1` to auto-enter the U-shield PIN. | USBHub port 10. BOC **never goes through a proxy** — `_disable_proxy_for_playwright` is load-bearing. Safety-mode entries: `python open_boc.py --open-login` only opens the login page (no UKey, no USBHub touch), `python open_boc.py --login-only` stops after login and does not enter 付款服务/转账汇款. |
...
Not a bank flow on its own, but consumed by the 制单 projects above:

- **`招行/招行制单/M3直供合同付款数据获取`** — Playwright pipeline that logs into 国瑞 OA (`newoa.guorui.net`), walks 集团空间 → 报表中心 → 财务报表 → 合同付款, and writes `bank_form.json` (招行制单 field shape) plus `latest.json` (raw OA fields). Run the `step1_open.py` → `step4_extract.py` sequence; see the project's `SKILL.md` for details. **Both 招行制单 and 农业银行 consume the produced `bank_form.json`** — 招行 reads it directly, 农业 reads it via `M3_TRANSFER_DATA_DIR` / `M3_BANK_FORM_PATH` in its `.env`. If you change the field schema, update both downstream readers. For 24h unattended running this pipeline adds: a per-bank subprocess timeout (`BANK_ROUTE_PROCESS_TIMEOUT_SECONDS`, default 900s, exit code 124 + per-bank cleanup) — **exit 124 is never auto-retried (ABC/BOC included)**: timeout may land in the grey zone where the voucher was already submitted into the bank's pending-review queue but the script didn't exit cleanly, so it is kept failed and needs manual review of that bank's pending-review queue before any re-run; other business exit codes keep their existing retry behavior unchanged. Crash recovery (`--resume-existing` / `M3_MONITOR_RESUME_EXISTING=1`, only re-runs interrupted `running` items — never bulk re-runs history), a watchdog `run_auto_extract_dispatch_生产_watchdog.ps1` (respects the monitor lock, never blind `--force-lock`), and an optional production single-payment cap `M3_MAX_PAYMENT_AMOUNT` (empty = format-check only). None of this changes the boundary — still no auto review/authorize/final payment. See that project's `SKILL.md` → "24 小时守护运行".
...
- **Fail closed on resource/transfer ambiguity.** No payer match, no `bank_form.json` (招行), placeholder `DEFAULT_TRANSFER` still in place (兴业), invalid amount, unmapped UKey port, or unexpected page state → abort before opening the bank, or before clicking 经办/下一步/提交. 招行 and 农业 document explicit exit codes (e.g. exit `2` for missing/invalid transfer data — see 招行 `SKILL.md` exit-code table and `农业/农业银行/CLAUDE.md`); 兴业 and 中行 fail-close via `sys.exit(1)` / `raise SystemExit("...")` with log messages instead of an enumerated table, so verification there means inspecting logs/screenshots rather than matching a code.
...
- **BOC must stop after the first-step bottom `提交`.** No automatic confirm/pay/authorize. This is a hard rule encoded in `中行/中国银行/CLAUDE.md` and `AGENTS.md`.
...
- ABC and BOC both run direct — `--no-proxy-server` plus proxy env-var cleanup at process startup. BOC is stricter: `boc_automation/proxy.py` additionally enforces a fake-ip preflight (refuses to continue if `netc1.igtb.boc.cn` / `netc2.igtb.boc.cn` resolve into `198.18.0.0/15`) and a TCP-443 direct-connect preflight. Do not add proxy fallbacks for either, even if a different bank's machine config goes through a proxy.
...
- After editing a project's contract, sync `银行自动化项目总览.md` in the same change. The overview is the single document that an outside operator — Codex, Claude, or a human — will look at first to decide which project to use and what it will actually do.
